...
// Preload background and covers

var z, w, loaderProgress = 0, counter = 0,
	counter2 = 0;

// start loading loop
function preloadImages() {
	
	for (z = 0; z <= covers.length; z++) {

		// if all covers loaded and bg are defined
		if (z === covers.length && !(data.coverIsAlsoBackground)) {

			for (w = 0; w <= bg.length; w++) {

				// if all bg are loaded
				if (w === bg.length) {
					...
					// else load the bg
					$('<img />').attr('src', ((data.pathToBg) + bg[w])).appendTo('body').css('display', 'none');

					loaderProgress = (w / bg.length) * 100;
		...
		} else if (z === covers.length) {

			// if all covers loaded
			$('#loader').delay(200).fadeOut(500);
			...
			// else load the cover
			$('<img />').attr('src', ((data.pathToCovers) + covers[z])).appendTo('body').css('display', 'none');

			loaderProgress = (z / covers.length) * 100;
...
//	console.log('not firefox or chrome');
...
$(document).ready(function () {
	...
	if (window.location.hash) {
		...
		if ((window.location.hash !== '#0')) {
			...
			if ((Number(window.location.hash.replace('#', '')) <= tracks.length)) {
	...
	$('.icon-heart').on('click', function () {
		...
		$('.share-container').load('share.html', function () {
			...
			$('#share-close').css({
				'fill': data.sharePageCloseColor
			});

			$('.social .overlay').on('mouseenter', function () {
	...
	$('.animation-svg').on('click', function () {
	...
	function updateHash() {
		...
		window.location.hash = Number(trackSelected);
	...
	function animation() {
      ...
      [4 / 4, [1.0, 1.0, 1.0, 1.0]]
		...
		loop(function (f, t, dt) {
			...
			var ampli = analyser._audio.duration ? Math.min(1, Math.pow(1.25 * analyser.amplitude(15e3), 2)) : 0.5 - 0.25 * Math.cos(t / 1000);

			var s = 1.01 + 0.09 * ampli;
			...
			for (var a, r, i = 0, j = analyser.wave.length; i < j; i++) {
				a = i / j * 2 * Math.PI;
				r = ampli * 256 * (0.5 + analyser.wave[i] / 255);
				$.lineTo(r * Math.sin(a), r * Math.cos(a));
			}

			var c = gradient(colors[0], ampli);
			c[3] = ampli;
			$.fillStyle = rgba(c);
